  ...
  vision_msgs__msg__BoundingBox3D__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vision_msgs__msg__BoundingBox3D * data =
      (vision_msgs__msg__BoundingBox3D *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vision_msgs__msg__BoundingBox3D__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          vision_msgs__msg__BoundingBox3D__fini(&data[i]);
        }
        free(data);
        return false;
      }
