function HelperDrawCuboid(ax, cuboids, color, modelInfo, velInfo, orientInfo)
    ...
    if ((size(modelInfo.Distance,1) == size(velInfo,1)) && (size(velInfo,1) == size(orientInfo,1))) && (length(cuboids) == length(modelInfo.Distance))
        % 각 cuboid에 대해 반복
        for i = 1:size(cuboids, 1)
           ...
           % 
            ...
            if size(velInfo,1) == size(cuboids,1)
                vx = velInfo(i,1);
                vy = velInfo(i,2);
                ...
                text(ax,[topCenter(1),      topCenter(1),       topCenter(1),                           topCenter(1)],...
                        ...
                        [topCenter(3)+1.5,  topCenter(3)+1,   topCenter(3)+0.5,                       topCenter(3)],...
                ...
                text(ax,[topCenter(1),       topCenter(1),       topCenter(1)],...
                        ...
                        [topCenter(3)+1,   topCenter(3)+0.5,   topCenter(3)],...
