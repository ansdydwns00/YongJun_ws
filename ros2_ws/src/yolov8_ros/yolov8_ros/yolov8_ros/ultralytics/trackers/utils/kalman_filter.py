...
# class ExtendedKalmanFilterXYAH:
#     """
#     An ExtendedKalmanFilterXYAH class for tracking bounding boxes in image space using an Extended Kalman filter.

#     Implements an Extended Kalman filter (EKF) for tracking bounding boxes in image space. The 8-dimensional state space
#     (x, y, a, h, vx, vy, va, vh) contains the bounding box center position (x, y), aspect ratio a, height h, and their
#     respective velocities. Object motion follows a constant velocity model, and bounding box location (x, y, a, h) is
#     taken as a direct observation of the state space (linear observation model).
...
#     Methods:
#         initiate: Creates a track from an unassociated measurement.
#         predict: Runs the EKF prediction step.
#         project: Projects the state distribution to measurement space using the observation model.
#         update: Runs the EKF correction step.
#         gating_distance: Computes the gating distance between state distribution and measurements.
#         multi_predict: Runs the EKF prediction step for multiple object states (vectorized version).
#     """

#     def __init__(self):
#         """
#         Initialize the EKF model with motion and observation uncertainty weights.
#         """
#         ndim, dt = 4, 1.0
...
#     def project(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
#         """
#         Project state distribution to measurement space using the observation model.
#         """
#         H = self._jacobian_observation(mean)
...
#         mean = np.dot(H, mean)
#         covariance = np.linalg.multi_dot((H, covariance, H.T)) + innovation_cov

#         return mean, covariance

#     def update(self, mean: np.ndarray, covariance: np.ndarray, measurement: np.ndarray) -> tuple:
#         """
#         Run EKF correction step.
#         """
#         H = self._jacobian_observation(mean)
#         projected_mean, projected_cov = self.project(mean, covariance)
...
#         """
#         Run EKF prediction step for multiple object states (Vectorized version).

#         Args:
#             means (ndarray): The Nx8-dimensional mean matrix of the object states at the previous time step.
#             covariances (ndarray): The Nx8x8 covariance matrix of the object states at the previous time step.

#         Returns:
#             (tuple[ndarray, ndarray]): Returns the mean matrix and covariance matrix of the predicted states.
#                 The mean matrix has shape (N, 8) and the covariance matrix has shape (N, 8, 8).
#         """
#         N = means.shape[0]

#         # Compute the Jacobian of the motion model for multiple states
#         F = np.eye(2 * self.ndim).repeat(N, axis=0).reshape(N, 2 * self.ndim, 2 * self.ndim)
#         for i in range(self.ndim):
#             F[:, i, self.ndim + i] = self.dt
...
#         # Predict means
#         means = np.einsum('ijk,ik->ij', F, means)

#         # Predict covariances
#         left = np.einsum('ijk,ikl->ijl', F, covariances)
#         covariances = np.einsum('ijk,ikl->ijl', left, F) + motion_cov
