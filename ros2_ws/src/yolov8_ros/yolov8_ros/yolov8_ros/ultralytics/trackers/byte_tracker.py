...
class STrack(BaseTrack):
    ...
    """
    # shared_kalman는 모든 STrack 인스터스에서 공유되는 kalmanFilterXYAH 객체
    shared_kalman = KalmanFilterXYAH()
    
    # __init__ 메소드는 STrack 클래스의 생성자, 새로운 인스터스를 초기화하는 역할 
    # xywh, score, cls 등의 인자를 받아 객체를 초기화
    # super().__init__()을 호출해 부모 클래스(BaseTrack)의 초기화 메소드를 실행
    ...
    def result(self):
        ...
        return coords.tolist() + [self.velocity[0], self.velocity[1], self.track_id, self.score, self.cls, self.idx]
