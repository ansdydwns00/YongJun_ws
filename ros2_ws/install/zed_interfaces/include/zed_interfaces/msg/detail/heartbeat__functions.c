  ...
  zed_interfaces__msg__Heartbeat__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    zed_interfaces__msg__Heartbeat * data =
      (zed_interfaces__msg__Heartbeat *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!zed_interfaces__msg__Heartbeat__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          zed_interfaces__msg__Heartbeat__fini(&data[i]);
        }
        free(data);
        return false;
      }
