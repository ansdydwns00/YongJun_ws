               ...
               int32_T bestModelParams_size[2])
{
  ...
  skipTrials = 0;
  emxInit_boolean_T(sp, &bestInliers, 1, &qc_emlrtRTEI);
  inb = bestInliers->size[0];
  bestInliers->size[0] = allPoints->size[0];
  emxEnsureCapacity_boolean_T(sp, bestInliers, inb, &qc_emlrtRTEI);
  bestInliers_data = bestInliers->data;
  for (inb = 0; inb < numPts_tmp; inb++) {
    bestInliers_data[inb] = false;
  }
  emxInit_real32_T(sp, &dis, 1, &sc_emlrtRTEI);
  while ((idxTrial <= numTrials) && (skipTrials < 10000)) {
    ...
    if (isValidModel) {
      ...
      if (angle < bestDis) {
        ...
        emxEnsureCapacity_boolean_T(sp, bestInliers, inb, &rc_emlrtRTEI);
        bestInliers_data = bestInliers->data;
        for (inb = 0; inb < nz; inb++) {
        ...
        if (angle < 1.1920929E-7F) {
          ...
          if (1.0F - angle < 0.0F) {
            emlrtErrorWithMessageIdR2018a(
                &b_st, &q_emlrtRTEI, "Coder:toolbox:ElFunDomainError",
