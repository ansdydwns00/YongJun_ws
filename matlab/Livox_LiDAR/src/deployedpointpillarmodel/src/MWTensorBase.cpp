...
void MWTensorBase::setDataFormat(const char* aformat) {
    setDataFormat(std::string(aformat));
}

void MWTensorBase::setDataFormat(std::string aformat) {
    assert(!aformat.empty());
    m_dlarray_dimsFormat = aformat;
}

// get Tensor's format
std::string MWTensorBase::getDataFormat() const {
    assert(!m_dlarray_dimsFormat.empty());
    return m_dlarray_dimsFormat;
}


bool MWTensorBase::hasTimeDim() const {
    // 'T' refers to Time dimension in dlarray_dimsFormat
    return hasDimsFormat('T');
}

bool MWTensorBase::hasSpatialDim() const {
    // 'S' refers to Spatial dimension in dlarray_dimsFormat
    return hasDimsFormat('S');
}

bool MWTensorBase::hasDimsFormat(const char dim) const {
    return (m_dlarray_dimsFormat.find(dim) != std::string::npos);
}

bool MWTensorBase::hasCBFormat() const {
    // We also check for CU case as featureInputLayer can accept "CU" input.
    // Layers after the featureInputLayer only propagate "CB" format.
    return ((m_dlarray_dimsFormat.compare("CB") == 0) || (m_dlarray_dimsFormat.compare("CU") == 0));
}
