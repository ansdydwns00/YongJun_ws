  ...
  vision_msgs__msg__Detection3DArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vision_msgs__msg__Detection3DArray * data =
      (vision_msgs__msg__Detection3DArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vision_msgs__msg__Detection3DArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          vision_msgs__msg__Detection3DArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
