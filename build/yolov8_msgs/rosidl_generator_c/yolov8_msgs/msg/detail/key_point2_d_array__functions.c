  ...
  yolov8_msgs__msg__KeyPoint2DArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yolov8_msgs__msg__KeyPoint2DArray * data =
      (yolov8_msgs__msg__KeyPoint2DArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yolov8_msgs__msg__KeyPoint2DArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          yolov8_msgs__msg__KeyPoint2DArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
